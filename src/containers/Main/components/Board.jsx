...
function Board({ data }) {
  ...
  const colorTotalDeaths = '#6B533D'
  const colorTodayCases = '#E3756B'
  const colorDeaths = '#000000'
  const colorRecovered =  '#6DD694'

  const getValue = (value) => value ? value : <Skeleton variant="text" width="100%" height={60} />
  ...
  return (
    <Grid container spacing={4}>
      ...
      <Grid item xs={12} md={3}>
        <Card value={getValue(deaths)} label=" Total de mortos" color={colorDeaths} icon={faBookDead} iconColor={colorDeaths}/>
      </Grid>
      <Grid item xs={12} md={3}>
        <Card value={getValue(recovered)} label=" Recuperados" color={colorRecovered} icon={faVirusSlash} iconColor={colorRecovered}/>
      </Grid>
    </Grid>
  )
}
